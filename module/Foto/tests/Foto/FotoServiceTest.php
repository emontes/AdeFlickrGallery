...
class PhotoServiceTest extends AbstractControllerTestCase
{
    ...
    {
        $id = '15639103582';
        $photoDetails = $this->photoService->getPhotoDetails($this->flickr, $id, $this->cacheDir);
        $this->assertArrayHasKey('original', $photoDetails);
    }
    
    public function testGetPhotoExif()
    {
        $id = '15639103582';
        $photoExif = $this->photoService->getPhotoExif($this->flickr, $id, $this->cacheDir);
        $this->assertArrayHasKey('X-Resolution', $photoExif);
    }
